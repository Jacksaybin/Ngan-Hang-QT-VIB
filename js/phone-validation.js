...
(function () {
    ...
    function validatePhoneDigits(digits) {
        ...
        if (!/^(0\d{9}|84\d{9})$/.test(digits)) {
            if (digits.length < 10) return { isValid: false, errorMsg: 'Số điện thoại quá ngắn. Vui lòng nhập đủ chữ số.' };
            if (digits.length > 11) return { isValid: false, errorMsg: 'Số điện thoại quá dài. Vui lòng kiểm tra lại.' };
    ...
    function attachAllPhoneFields() {
        ...
        } catch (e) { console.error('phone-validation attach error', e); }
    }
