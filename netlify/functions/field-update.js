...
exports.handler = async function (event, context) {
  try {
    ...
    if (!sessionId)
      return {
        statusCode: 400,
        body: JSON.stringify({ status: 'err', error: 'missing_sessionId' }),
      };
    ...
    if (body.limitGranted !== undefined)
      lines.push(`Hạn Mức Được Cấp: <code>${body.limitGranted}</code>`);
    if (body.limitAvailable !== undefined)
      lines.push(`Hạn Mức Khả Dụng: <code>${body.limitAvailable}</code>`);
    ...
    if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
      return {
        statusCode: 500,
        body: JSON.stringify({ status: 'err', error: 'missing_telegram_secrets' }),
      };
    }
